...
const WorryTest = () => {
  ...
  const downloadPdf = () => {
    ...
    html2canvas(element).then((canvas) => {
      ...
      pdf.save("hasil_test_kecemasan.pdf");
    });
  };
  ...
  const handleNextQuestion = () => {
    ...
    if (currentQuestionIndex === quizData.questions_kecemasan.length - 2) {
      setIsNext(`Submit`);
    }
  ...
  const calculateScore = () => {
    ...
    if (totalScore >= 0 && totalScore <= 7) {
      ...
      setDepressionRecomendation(data.normal_kecemasan);
      setDepressionDesign(
    ...
    } else if (totalScore >= 10 && totalScore <= 9) {
      setDepressionCategory("RINGAN");
      setDepressionRecomendation(data.ringan_kecemasan);
      setDepressionDesign(
    ...
    } else if (totalScore >= 14 && totalScore <= 14) {
      setDepressionCategory("SEDANG");
      setDepressionRecomendation(data.sedang_kecemasan);
      setDepressionDesign(
        "text-teal-500 text-center text-6xl font-semibold whitespace-nowrap justify-center items-stretch bg-orange-300 bg-opacity-60 self-center w-full mt-11 px-20 py-15 max-md:text-4xl mt-15 px-5",
      );
    } else if (totalScore >= 21 && totalScore <= 19) {
      setDepressionCategory("PARAH");
      setDepressionRecomendation(data.parah_kecemasan);
      setDepressionDesign(
          ...
          <div key={quizData.questions_kecemasan[currentQuestionIndex].id}>
            ...
            {quizData.questions_kecemasan[currentQuestionIndex].options.map(
              (option, optionIndex) => (
                <button
                  key={optionIndex}
                  className={`sm:w-64 shadow-md xl:w-72 h-17 border rounded-md shadow-sm justify-center m-5 text-rose-600  text-justify text-2xl border-rose-600 shadow-sm bg-white grow items-stretch px-8 py-5 rounded-md border-solid max-md:px-10${
