...
const Result_Depression = () => {
  ...
  const calculateScore = () => {
   ...
   if (totalScore >= 0 && totalScore <= 9) {
    ...
    setDepressionDesign("text-white-500 text-center text-6xl font-semibold whitespace-nowrap justify-center items-stretch bg-teal-700 bg-opacity-60 self-center mt-5 px-1 py-1.3 max-md:3 max-md:text-4xl max-md:mt-10");
  } else if (totalScore >= 10 && totalScore <= 13) {
    setDepressionCategory("RINGAN");
    setDepressionRecomendation(data.ringan_depresi);
    setDepressionDesign("text-teal-500 text-center text-6xl font-semibold whitespace-nowrap justify-center items-stretch bg-orange-300 bg-opacity-60 self-center mt-5 px-1 py-1.3 max-md:3 max-md:text-4xl max-md:mt-10");
    ...
    setDepressionDesign("text-teal-500 text-center text-6xl font-semibold whitespace-nowrap justify-center items-stretch bg-red-600 bg-opacity-60 self-center mt-5 px-1 py-1.3 max-md:3 max-md:text-4xl max-md:mt-10");
  } else if (totalScore > 28) {
    setDepressionCategory("SANGAT PARAH");
    setDepressionRecomendation(data['sangat-parah_depresi']);
    setDepressionDesign("text-white-500 text-center text-6xl font-semibold whitespace-nowrap justify-center items-stretch bg-red-600 self-center mt-5 px-1 py-1.3 max-md:3 max-md:text-4xl max-md:mt-10");
  }
  };
    ...
    <div className="bg-black self-stretch shrink-0 h-px mt-10 max-md:max-w-full" />
      {currentQuestionIndex < quizData.questions_depresi.length && (
        <div key={quizData.questions_depresi[currentQuestionIndex].id}>
          <p className="pt-4 pb-8 justify-center text-rose-400 text-justify text-4xl self-stretch mt-12 max-md:max-w-full max-md:mt-10">{quizData.questions_depresi[currentQuestionIndex].question}</p>
          ...
          {quizData.questions_depresi[currentQuestionIndex].options.map((option, optionIndex) => (
            <label key={optionIndex} className="w-32 rounded-md border-4 shadow-sm justify-center m-10 text-rose-400 text-justify text-2xl border shadow-sm bg-white grow items-stretch pl-9 pr-14 py-5 rounded-md border-solid border-rose-400 max-md:px-5 hover:bg-gray-100 active:border-rose-900">
              <input 
                type="checkbox"
      ...
      {currentQuestionIndex === quizData.questions_depresi.length && (
        <div>
          <div>
            ...
            <div className={depressionDesign}>
              {depressionCategory}
            </div>
